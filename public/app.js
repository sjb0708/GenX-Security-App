...
// ── Word export ──────────────────────────────────────────────────────────────
// Word's HTML engine has no support for CSS grid or flexbox, so exporting the
// on-screen markup collapses every column into a stack. This builds the
// document from the brief DATA using tables and inline styles — the only
// layout primitives Word reproduces reliably — so it opens looking like a
// document rather than a pile of paragraphs.
...
function wSection(title) {
  return `<p style="margin:18pt 0 6pt 0;padding:0 0 3pt 0;border-bottom:1.5pt solid ${W.red};">
    <span style="font-family:Calibri,Arial,sans-serif;font-size:10pt;font-weight:bold;color:${W.red};letter-spacing:1pt;text-transform:uppercase;">${esc(title)}</span></p>`;
}

// Label/value pairs as a real two-column table — Word keeps these aligned.
function wKvTable(pairs) {
  const rows = pairs.filter(p => isContentValue(p[1])).map(([k, v]) => `
    <tr>
      <td style="width:1.6in;padding:4pt 8pt 4pt 0;border-bottom:0.5pt solid ${W.hair};vertical-align:top;">
        <span style="font-size:8pt;font-weight:bold;color:${W.soft};text-transform:uppercase;">${esc(k)}</span></td>
      <td style="padding:4pt 0;border-bottom:0.5pt solid ${W.hair};vertical-align:top;">
        <span style="font-size:10pt;color:${W.ink};">${esc(String(v))}</span></td>
    </tr>`).join('');
  return rows ? `<table style="width:100%;border-collapse:collapse;">${rows}</table>` : '';
}

function wNote(label, body) {
  if (!isContentValue(body)) return '';
  return `<p style="margin:8pt 0 2pt 0;"><span style="font-size:8pt;font-weight:bold;color:${W.soft};text-transform:uppercase;">${esc(label)}</span></p>
    <table style="width:100%;border-collapse:collapse;"><tr>
      <td style="border-left:2pt solid ${W.red};background:${W.wash};padding:7pt 9pt;">
        <span style="font-size:10pt;color:${W.mid};">${esc(body).replace(/\n/g, '<br>')}</span></td>
    </tr></table>`;
}

function wDataTable(headers, rows) {
  if (!rows.length) return '';
  const th = headers.map(h => `<td style="background:${W.red};padding:5pt 8pt;"><span style="font-size:8pt;font-weight:bold;color:#ffffff;text-transform:uppercase;">${esc(h)}</span></td>`).join('');
  const tr = rows.map(r => `<tr>${r.map(c => `<td style="padding:5pt 8pt;border-bottom:0.5pt solid ${W.hair};vertical-align:top;"><span style="font-size:9.5pt;color:${W.ink};">${c}</span></td>`).join('')}</tr>`).join('');
  return `<table style="width:100%;border-collapse:collapse;margin-top:4pt;"><tr>${th}</tr>${tr}</table>`;
}

// People as a fixed-column table so Word lays the cards out in a grid.
function wPeopleTable(people, cols = 4) {
  if (!people.length) return '';
  const cells = people.map(p => `
    <td style="width:${Math.floor(100 / cols)}%;padding:6pt;border:0.5pt solid ${W.line};vertical-align:top;text-align:center;">
      ${p.photo ? `<img src="${esc(p.photo)}" width="86" height="86" style="width:86px;height:86px;"><br>` : ''}
      <span style="font-size:9.5pt;font-weight:bold;color:${W.ink};">${esc(p.name || '')}</span><br>
      <span style="font-size:8.5pt;color:${W.soft};">${esc(p.role || p.function || '')}</span>
      ${p.phone ? `<br><span style="font-size:8.5pt;color:${W.soft};">${esc(p.phone)}</span>` : ''}
      ${p.email ? `<br><span style="font-size:8pt;color:${W.soft};">${esc(p.email)}</span>` : ''}
    </td>`);
  let out = '';
  for (let i = 0; i < cells.length; i += cols) {
    const row = cells.slice(i, i + cols);
    while (row.length < cols) row.push(`<td style="border:none;"></td>`);
    out += `<tr>${row.join('')}</tr>`;
  }
  return `<table style="width:100%;border-collapse:collapse;margin-top:4pt;">${out}</table>`;
}

// The brief stores some answers as booleans and some as "yes"/"no" strings.
function yn(v) {
...
function buildWordHtml(b) {
  ...
  parts.push(`<p style="margin:0 0 2pt 0;"><span style="font-size:9pt;font-weight:bold;color:${W.red};letter-spacing:1.5pt;">SECURITY BRIEF — CONFIDENTIAL</span></p>
    <p style="margin:0 0 6pt 0;"><span style="font-size:24pt;font-weight:bold;color:${W.ink};">${esc(v.name || 'Security Brief')}</span></p>
    <p style="margin:0 0 14pt 0;"><span style="font-size:9.5pt;color:${W.soft};">Prepared by GenX Corporate Security · ${esc(new Date().toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' }))}</span></p>`);

  parts.push(wSection('Venue'));
  parts.push(wKvTable([['Location', v.name], ['Business Name', v.businessName], ['Address', addr(v)],
    ['Phone', v.phone], ['Capacity', v.capacity], ['Type', v.type]]));
  parts.push(wNote('Security Concerns — Past 12 Months', v.priorIncidents));

  // Same rule the brief view uses: when the hotel address matches the venue,
  // the talent is lodging on site — say so instead of repeating the address.
  ...
  if (sameAsVenue) {
    parts.push(`<p style="margin:8pt 0 0 0;"><span style="font-size:10pt;font-style:italic;color:${W.mid};">Talent lodging on-site at venue${h.checkin ? ' · Check-in ' + esc(formatDate(h.checkin)) : ''}${h.checkout ? ' · Check-out ' + esc(formatDate(h.checkout)) : ''}</span></p>`);
  } else if (hasHotel) {
    parts.push(wSection('Hotel'));
    parts.push(wKvTable([['Name', h.name], ['Address', addr(h)], ['Phone', h.phone],
      ['Check-In', h.checkin ? formatDate(h.checkin) : ''], ['Check-Out', h.checkout ? formatDate(h.checkout) : '']]));
  }

  parts.push(wSection('Event Timeline'));
  parts.push(wKvTable([
    ['Arrival',   tl.arrivalDateTBD ? 'TBD' : (tl.arrivalDate ? formatDate(tl.arrivalDate) : '')],
    ['Show Day',  tl.showDateTBD ? 'TBD' : (tl.showDate ? formatDate(tl.showDate) : '')],
    ['Show Time', tl.showTimeTBD ? 'TBD' : (tl.showTime ? formatTime(tl.showTime) : '')],
    ['Doors',     tl.doorsTimeTBD ? 'TBD' : (tl.doorsTime ? formatTime(tl.doorsTime) : '')],
    ['Departure', tl.departureDateTBD ? 'TBD' : (tl.departureDate ? formatDate(tl.departureDate) : '')]]));
  parts.push(wNote('Timeline Notes', tl.notes));

  parts.push(wSection('Security Contacts'));
  const person = o => o ? [o.name, o.title || o.role, o.cell || o.phone, o.email].filter(Boolean).join(' · ') : '';
  parts.push(wKvTable([
    ['Primary', person(ct.primary)],
    ['Backup',  person(ct.backup)],
    ['Officers On-Site', yn(ct.leOnSite)], ['Agency', ct.leAgency]]));

  parts.push(wSection('Ingress & Screening'));
  const screening = [['evolv','Evolv'],['magnetometer','Magnetometer'],['wand','Wand'],
    ['bagCheck','Bag check'],['patDown','Pat down'],['visualInspection','Visual inspection']]
    .filter(([k]) => ing[k]).map(([, lbl]) => lbl).join(', ');
  parts.push(wKvTable([['Methods', screening], ['Ticketing', ing.ticketingType],
  ...
  parts.push(wNote('Ingress Notes', ing.notes));

  parts.push(wSection('Staffing'));
  parts.push(wKvTable([['Total Security', st.totalSecurity], ['LEO', st.leo],
    ['Backstage', st.backstageSecurity], ['GenX Security', st.genxSecurity],
    ['Uniformed', yn(st.uniformed)], ['Uniform', st.uniformDesc]]));
  parts.push(wNote('Staffing Notes', st.notes));

  parts.push(wSection('Medical & Emergency'));
  parts.push(wKvTable([['Trained Medical Staff', yn(med.onSite)],
    ['Responder Count', med.firstResponderCount],
    ['First Aid Locations', med.firstAidLocations],
    ['AED On Site', yn(med.aedOnSite)], ['AED Near Stage', yn(med.aedNearStage)],
    ['Medical to Green Room', yn(med.toGreenRoom)]]));
  parts.push(wNote('Emergency Protocol', med.emergencyProtocol));

  parts.push(wSection('Evacuation'));
  parts.push(wKvTable([['Primary Exit', ev.primaryExit], ['Secondary Exit', ev.secondaryExit],
    ['Rally Point', ev.rallyPoint], ['Safe Rooms', (ev.safeRooms || []).join(', ')]]));
  parts.push(wNote('EAP Notes', ev.eapNotes));
  parts.push(wNote('Lockdown Protocol', ev.lockdownProtocol));

  parts.push(wSection('Meet & Greet'));
  parts.push(wKvTable([['Scheduled', yn(mg.scheduled)], ['Time', mg.time],
    ['Duration', mg.duration], ['Location', mg.location],
    ['VIPs', mg.totalVips], ['GenX Staff', mg.genxStaff]]));
  parts.push(wNote('M&G Protocol', mg.protocol));
  parts.push(wNote('Gift Policy', mg.giftPolicy));

  parts.push(wSection('Communications'));
  parts.push(wKvTable([['Venue Shares Comms', yn(comm.venueShareComms)],
    ['Ops Center On Site', yn(comm.opsCenterOnSite)], ['Security Operations', comm.securityOps],
    ['Ops Phone', comm.securityOpsPhone], ['Cell Coverage OK', yn(comm.cellOk)]]));
  const chans = (comm.channels || []).filter(c => c && (c.num || c.use));
  if (chans.length) parts.push(wDataTable(['Ch', 'Use'], chans.map(c => [esc(String(c.num || '')), esc(c.use || '')])));
  parts.push(wNote('Comms Notes', comm.notes));

  parts.push(wSection('Access Control'));
  const creds = (ac.credentials || []).filter(c => c && c.name);
  if (creds.length) parts.push(wDataTable(['Credential', 'Color', 'Access Level', 'Location'],
    creds.map(c => [esc(c.name || ''), esc(c.color || ''), esc(c.level || ''), esc(c.location || '')])));
  const gc = ac.genxCred || {};
  if (gc.frontImage || gc.backImage) {
    parts.push(`<p style="margin:10pt 0 4pt 0;"><span style="font-size:8pt;font-weight:bold;color:${W.soft};text-transform:uppercase;">GenX Security Credentials${gc.name ? ' — ' + esc(gc.name) : ''}</span></p>
      <table style="border-collapse:collapse;"><tr>
        ${gc.frontImage ? `<td style="padding:0 12pt 0 0;"><img src="${esc(gc.frontImage)}" width="150" style="width:150px;"></td>` : ''}
        ${gc.backImage ? `<td style="padding:0;"><img src="${esc(gc.backImage)}" width="150" style="width:150px;"></td>` : ''}
      </tr></table>`);
    if (gc.notes) parts.push(`<p style="margin:4pt 0;"><span style="font-size:9.5pt;color:${W.mid};">${esc(gc.notes)}</span></p>`);
  }
  parts.push(wKvTable([['Door Systems', (ac.doorSystems || []).join(', ')],
    ['Backstage Access-Controlled', yn(ac.backstageControlled)],
    ['Additional Credentials', ac.additionalCredentials]]));
  parts.push(wNote('Cast & Crew Access', ac.castCrewAccess));
  parts.push(wNote('GenX Arrival — Door / Meet / Time', ac.teamArrival));
  parts.push(wNote('Parking Notes', ac.parkingNotes));

  parts.push(wSection('Load In / Load Out'));
  parts.push(wKvTable([['Load In Date', li.loadinDate ? formatDate(li.loadinDate) : ''],
    ['Load In Time', li.loadinTime ? formatTime(li.loadinTime) : ''], ['Dock', li.dockLocation],
    ['Load Out Date', li.loadoutDate ? formatDate(li.loadoutDate) : ''],
    ['Load Out Time', li.loadoutTime ? formatTime(li.loadoutTime) : ''], ['Vehicles', li.vehicleCount]]));
  parts.push(wNote('Load In Notes', li.loadinNotes));
  parts.push(wNote('Load Out Notes', li.loadoutNotes));

  const days = rosNormalizeDays(b.runofshow || []).filter(d => d.rows.length);
  if (days.length) {
    parts.push(wSection('Run of Show'));
    days.forEach(d => {
      if (days.length > 1) parts.push(`<p style="margin:10pt 0 2pt 0;"><span style="font-size:9pt;font-weight:bold;color:${W.red};text-transform:uppercase;">${esc(d.label)}</span></p>`);
      parts.push(wDataTable(['Time', 'Activity', 'Security Notes'],
        d.rows.map(r => [esc(r.time ? formatTime(r.time) : ''), esc(r.activity || ''), esc(r.notes || '')])));
    });
  }

  if ((b.talent || []).length)    { parts.push(wSection('Talent'));                parts.push(wPeopleTable(b.talent)); }
  if ((b.crew || []).length)      { parts.push(wSection('Crew & Production'));     parts.push(wPeopleTable(b.crew)); }
  if ((b.genxstaff || []).length) { parts.push(wSection('GenX Security Staff'));   parts.push(wPeopleTable(b.genxstaff)); }

  if ((b.emergency || []).length) {
    parts.push(wSection('Emergency Contacts'));
    parts.push(wDataTable(['Role', 'Name', 'Phone', 'Email'],
      b.emergency.map(e => [esc(e.role || ''), esc(e.name || ''), esc(e.phone || ''), esc(e.email || '')])));
  }

  const maps = (b.maps || []).filter(m => m.image);
  if (maps.length) {
    parts.push(wSection('Venue Maps & Diagrams'));
    maps.forEach(m => parts.push(`<p style="margin:8pt 0 3pt 0;"><span style="font-size:10pt;font-weight:bold;color:${W.ink};">${esc(m.title || '')}</span></p>
      <p style="margin:0 0 10pt 0;"><img src="${esc(m.image)}" width="620" style="width:620px;"></p>`));
  } else if (b.noMapsProvided) {
    parts.push(wSection('Venue Maps & Diagrams'));
    parts.push(`<p><span style="font-size:10pt;font-style:italic;color:${W.mid};">No maps provided to security team</span></p>`);
  }
...
async function downloadWordDoc() {
  ...
  try {
    const body = await inlineWordImages(buildWordHtml(b));
    const name = (b.venue?.name || 'Security Brief').replace(/[\\/:*?"<>|]/g, '-');
    const html = `<html xmlns:o="urn:schemas-microsoft-com:office:office" xmlns:w="urn:schemas-microsoft-com:office:word" xmlns="http://www.w3.org/TR/REC-html40">
<head><meta charset="utf-8"><title>${esc(name)} — Security Brief</title>
<style>
@page WordSection1 { size: 8.5in 11.0in; margin: 0.8in 0.7in 0.8in 0.7in; }
div.WordSection1 { page: WordSection1; }
body { font-family: Calibri, Arial, sans-serif; font-size: 10.5pt; color: ${W.ink}; }
...
<body><div class="WordSection1">${body}</div></body></html>`;
    ...
    a.href = url; a.download = `${name} — Security Brief.doc`;
